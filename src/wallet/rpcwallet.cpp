...
UniValue setlabel(const Config &config, const JSONRPCRequest &request) {
    ...
    std::string dest_label = request.params[0].get_str();
    CTxDestination dest = DecodeDestination(dest_label, config.GetChainParams());
...
static UniValue sendtoaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The devault address "
            "to send to.\n"
            ...
            "devaults than you enter in the amount field.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n" +
            HelpExampleCli("sendtoaddress",
                           "\"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\" 0.1") +
            HelpExampleCli("sendtoaddress", "\"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9"
...
static UniValue getreceivedbyaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "confirmations\n" +
            HelpExampleCli("getreceivedbyaddress",
                           "\"devault:qpzfppqqg5sk6ck8c624tk7vgxeuafaq9uumff5u2u\" 0") +
            "\nThe amount with at least 6 confirmations\n" +
            HelpExampleCli("getreceivedbyaddress",
                           "\"devault:qpzfppqqg5sk6ck8c624tk7vgxeuafaq9uumff5u2u\" 6") +
            "\nAs a json rpc call\n" +
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The devault address "
            "to send funds to.\n"
            ...
            HelpExampleCli("sendfrom",
                           "\"tabby\" \"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\" "
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The devault "
            ...
            "devaults than you enter in their corresponding amount field.\n"
            ...
            HelpExampleCli("sendmany",
                           "\"\" "
                           "\"{\\\"devault:qpzfppqqg5sk6ck8c624tk7vgxeuafaq9uumff5u2u\\\":0.01,"
                           "\\\"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\\\":0.02}"
                           "\"") +
            ...
            HelpExampleCli("sendmany",
                           ...
                           "1 \"\" "
                           "\"[\\\"devault:qpzfppqqg5sk6ck8c624tk7vgxeuafaq9uumff5u2u\\\","
                           "\\\"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\\\"]\"") +
            "\nAs a json rpc call\n" +
...
UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "2. \"keys\"         (string, required) A json array of devault "
            "addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) devault address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
            ...
            HelpExampleCli("addmultisigaddress",
                           "2 "
                           "\"[\\\"devault:qplxfq6jfqappmgemvgqup64mg592rh2ssyv8zcze7\\\","
                           "\\\"devault:qpzfppqqg5sk6ck8c624tk7vgxeuafaq9uumff5u2u\\\"]\"") +
            "\nAs json rpc call\n" +
...
UniValue ListReceived(const Config &config,interfaces::Chain::Lock &locked_chain, CWallet *const pwallet,
                      const UniValue &params, bool by_label) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet) {
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            HelpExampleRpc(
                ...
                "6, true, true, \"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\""));
    }
...
UniValue listtransactions(const Config &config, const JSONRPCRequest &request) {
    ...
    for (auto& it : reverse_iterate(txOrdered)) {
        ...
        if (pwtx != nullptr) {
            ListTransactions(*locked_chain, pwallet, *pwtx, strAccount, 0, true,
                             ret, filter);
        }
...
static UniValue listsinceblock(const Config &config,
    ...
    for (const auto &pairWtx : pwallet->mapWallet) {
        ...
        if (depth == -1 || tx.GetDepthInMainChain(*locked_chain) < depth) {
            ListTransactions(*locked_chain, pwallet, tx, "*", 0, true,
                             transactions, filter);
        }
    }
    ...
    while (include_removed && paltindex && paltindex != pindex) {
        ...
        for (const CTransactionRef &tx : block.vtx) {
            ...
            if (it != pwallet->mapWallet.end()) {
                ...
                // negative.
                ListTransactions(*locked_chain, pwallet, it->second, "*",
...
static UniValue gettransaction(const Config &config,
    ...
    ListTransactions(*locked_chain, pwallet, wtx, "*", 0, false, details,
                     filter);
...
static UniValue walletpassphrase(const Config &config,
    ...
    if ((request.fHelp || request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "private keys such as sending devaults\n"
            "\nArguments:\n"
...
static UniValue walletlock(const Config &config,
    ...
    if ((request.fHelp || request.params.size() != 0)) {
        throw std::runtime_error(
            ...
            HelpExampleCli("sendtoaddress",
                           "\"devault:qp4au0wyh9zj2jchy96glxj9fsng8ru87ugcfylpc9\" 1.0") +
...
static UniValue lockunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "selection, when spending devaults.\n"
    ...
    for (size_t idx = 0; idx < output_params.size(); idx++) {
        ...
        if (pwallet->IsSpent(*locked_chain, outpt)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
...
static UniValue createwallet(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "wallet. A blank wallet has no keys or HD seed. "
            "\nResult:\n"
            "{\n"
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 5) {
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of devault addresses "
            "to filter\n"
            "    [\n"
            "      \"address\"     (string) devault address\n"
            "      ,...\n"
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The devault address to receive the change\n"
...
UniValue getaddressinfo(const Config &config, const JSONRPCRequest &request) {
    ...
    if (std::holds_alternative<CKeyID>(dest)) {
      auto key_id = std::get<CKeyID>(dest);
      auto it = pwallet->mapKeyMetadata.find(key_id);
      if (it != pwallet->mapKeyMetadata.end()) {
        meta = &it->second;
      }
    } else if (std::holds_alternative<BKeyID>(dest)) {
      auto key_id = std::get<BKeyID>(dest);
      auto it = pwallet->mapBLSKeyMetadata.find(key_id);
      if (it != pwallet->mapBLSKeyMetadata.end()) {
        meta = &it->second;
      }
    }
...
static const ContextFreeRPCCommand commands[] = {
    ...
    { "wallet",             "loadwallet",                   loadwallet,                   {"filename", "password"} },
