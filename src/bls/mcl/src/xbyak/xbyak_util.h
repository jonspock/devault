...
public:
	...
	*/
	...
	static inline uint64 getXfeature()
	{
		...
		return _xgetbv(0);
	#else
		unsigned int eaxx, edxx;
		// xgetvb is not support on gcc 4.2
//		__asm__ volatile("xgetbv" : "=a"(eaxx), "=d"(edxx) : "c"(0));
		__asm__ volatile(".byte 0x0f, 0x01, 0xd0" : "=a"(eaxx), "=d"(edxx) : "c"(0));
		return ((uint64)edxx << 32) | eaxx;
	#endif
#else
		return 0;
#endif
...
public:
	...
	#else
		unsigned int eaxx, edxx;
		__asm__ volatile("rdtsc" : "=a"(eaxx), "=d"(edxx));
		return ((uint64)edxx << 32) | eaxx;
	#endif
#else
