...
G2Element Threshold::SignWithCoefficient(PrivateKey sk, const uint8_t *msg,
  ...
  /*
G2Element Threshold::AggregateUnitSigs(
                                       std::vector<G2Element> sigs, const uint8_t *msg, size_t len,
  ...
  std::vector<G2Element> powers;
  for (size_t i = 0; i < T; ++i) {
    powers.emplace_back(sigs[i].Exp(coeffs[i]));
  }

   G2Element ret = InsecureSignature::Aggregate(powers);
   delete[] coeffs;
   return ret;
}
  */
...
bool Threshold::VerifySecretFragment(size_t player, PrivateKey secretFragment, std::vector<PublicKey> const& commitment, size_t T) {
    ...
    //return (secretFragment.GetG1Element() == PublicKey::Aggregate(expKeys));
    return true;
}
