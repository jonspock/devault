...
void CTxMemPool::UpdateTransactionsFromBlock(
    ...
    for (const TxId &txid : reverse_iterate(txidsToUpdate)) {
        ...
        for (; iter != mapNextTx.end() && iter->first->GetTxId() == txid;
            ...
            auto childIter = mapTx.find(childTxId);
...
bool CTxMemPool::CalculateMemPoolAncestors(
    ...
    if (fSearchForParents) {
        ...
        for (const CTxIn &in : tx.vin) {
            auto piter = GetIter(in.prevout.GetTxId());
            if (!piter) {
                continue;
            }
            parentHashes.insert(piter.value());
 ...
 bool CTxMemPool::removeAddrIndex(const uint256 txhash)
{
    ...
    auto it = mapAddrInserted.find(txhash);
     if (it != mapAddrInserted.end()) {
        ...
        for (auto& mit : keys) mapAddr.erase(mit);
...
void CTxMemPool::removeRecursive(const CTransaction &origTx,
    ...
    auto origit = mapTx.find(origTx.GetId());
    if (origit != mapTx.end()) {
        ...
        for (size_t i = 0; i < origTx.vout.size(); i++) {
            ...
            auto nextit = mapTx.find(it->second->GetId());
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin();
        ...
        if (!ContextualCheckTransactionForCurrentBlock(config.GetChainParams().GetConsensus(), tx, state,
            ...
            for (const CTxIn &txin : tx.vin) {
                auto it2 = mapTx.find(txin.prevout.GetTxId());
                if (it2 != mapTx.end()) {
                    continue;
                }
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin();
        ...
        for (; iter != mapNextTx.end() &&
             ...
             ++iter) {
            auto childit = mapTx.find(iter->second->GetId());
...
bool CTxMemPool::CompareDepthAndScore(const TxId &txida, const TxId &txidb) {
    LOCK(cs);
    auto i = mapTx.find(txida);
    if (i == mapTx.end()) {
        return false;
    }
    auto j = mapTx.find(txidb);
    if (j == mapTx.end()) {
        return true;
    }
...
void CTxMemPool::addUnchecked(const TxId &txid,
...
bool CTxMemPool::TransactionWithinChainLimit(const TxId &txid,
