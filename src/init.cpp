...
void SetupServerArgs() {
    ...
    // alphabetical ordering. Do not translate ...) -help-debug options, Many
    ...
    gArgs.AddArg(
                 "-keeplogfiles=<days>",
                 strprintf(
                           "If specified, debug log file older than <days> days will be deleted where days must be 1 or more, otherwise log files older than %d days will be deleted", DEFAULT_DEBUGLOGFILE_KEEPDAYS),
        false, OptionsCategory::OPTIONS);

   gArgs.AddArg("-maxmempool=<n>",
                 strprintf("Keep the transaction memory pool "
                             "below <n> megabytes (default: %u)",
                           DEFAULT_MAX_MEMPOOL_SIZE),
                 false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-maxorphantx=<n>",
                 strprintf("Keep at most <n> unconnectable "
                             "transactions in memory (default: %u)",
                           DEFAULT_MAX_ORPHAN_TRANSACTIONS),
                 false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-mempoolexpiry=<n>",
                 strprintf("Do not keep transactions in the mempool "
    ...
    gArgs.AddArg(
        ...
        false, OptionsCategory::OPTIONS);
#endif
    gArgs.AddArg("-addressindex", strprintf("Maintain a full address index, used to query for the balance,"
                                              " txids and unspent outputs for addresses (default: %u)", DEFAULT_ADDRESSINDEX), false, OptionsCategory::OPTIONS);
#ifdef EXTRA_INDEXES    
    gArgs.AddArg("-timestampindex", strprintf("Maintain a timestamp index for block hashes,"
                                                " used to query blocks hashes by a range of timestamps (default: %u)"), DEFAULT_TIMESTAMPINDEX));
    gArgs.AddArg("-spentindex", strprintf("Maintain a full spent index, used to query the spending txid "
    ...
    gArgs.AddArg(
        "-whitelistforcerelay",
        strprintf("Force relay of transactions from whitelisted peers even "
    ...
    gArgs.AddArg(
        "-minrelaytxfee=<amt>",
        strprintf(
            "Fees (in %s/kB) smaller than this are considered zero fee for "
...
std::string LicenseInfo() {
    ...
    return CopyrightHolders(
           ...
           "\n" + "\n" +
           strprintf(_("Please contribute if you find devault-qt useful. "
...
void CleanupBlockRevFiles() {
    ...
    for (const auto &item : mapBlockFiles) {
        if (std::atoi(item.first.c_str()) == contiguousCounter) {
            contiguousCounter++;
            continue;
        }
        remove(item.second);
...
bool AppInitSanityChecks() {
    ...
    if (!InitSanityCheck()) {
        return InitError(strprintf(
            _("Initialization sanity check failed. devault-qt is shutting down.")));
    }
