...
TEST_CASE("script_standard_Solver_success") {
  ...
  s.clear();
  s << OP_2 << ToByteVector(pubkeys[0]) << ToByteVector(pubkeys[1]) << ToByteVector(pubkeys[2]) << OP_3
    << OP_CHECKMULTISIG;
  BOOST_CHECK(Solver(s, solutions));
  BOOST_CHECK_EQUAL(Solver(s, solutions), TX_MULTISIG);
...
TEST_CASE("script_standard_Solver_failure") {
  BasicTestingSetup setup;
  CKey key;
  CPubKey pubkey;
  key.MakeNewKey();
...
TEST_CASE("script_standard_ExtractDestination") {
  BasicTestingSetup setup;
  CKey key;
  CPubKey pubkey;
  key.MakeNewKey();
  ...
  //    BOOST_CHECK(boost::get<CKeyID>(&address) && *boost::get<CKeyID>(&address) == pubkey.GetKeyID());
...
TEST_CASE("script_standard_ExtractDestinations") {
  BasicTestingSetup setup;
  CKey keys[3];
  CPubKey pubkeys[3];
  for (int i = 0; i < 3; i++) {
  ...
  BOOST_CHECK(std::get<CKeyID>(addresses[0]) == pubkeys[0].GetKeyID());
  ...
  BOOST_CHECK_EQUAL(addresses.size(), 1);
  BOOST_CHECK_EQUAL(nRequired, 1);
  BOOST_CHECK(std::get<CScriptID>(addresses[0]) ==  CScriptID(redeemScript));
  ...
  BOOST_CHECK(std::get<CKeyID>(addresses[0]) == pubkeys[0].GetKeyID());
  BOOST_CHECK(std::get<CKeyID>(addresses[1]) == pubkeys[1].GetKeyID());
...
TEST_CASE("script_standard_GetScriptFor_") {
  BasicTestingSetup setup;
  CKey keys[3];
  CPubKey pubkeys[3];
  for (int i = 0; i < 3; i++) {
  ...
  expected.clear();
  expected << OP_2 << ToByteVector(pubkeys[0]) << ToByteVector(pubkeys[1]) << ToByteVector(pubkeys[2]) << OP_3
           << OP_CHECKMULTISIG;
  result = GetScriptForMultisig(2, std::vector<CPubKey>(pubkeys, pubkeys + 3));
  BOOST_CHECK(result == expected);
}

TEST_CASE("script_standard_IsMine") {
  BasicTestingSetup setup;
  CKey keys[2];
  CPubKey pubkeys[2];
  for (int i = 0; i < 2; i++) {
