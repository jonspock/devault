...
TEST_CASE("TestPackageAccounting") {
  ...
  for (size_t i = 0; i < maxOutputs; i++) {
    parentOfAll.vout.emplace_back(10 * Amount::min_amount(), CScript() << OP_TRUE);
  }
  ...
  for (size_t totalTransactions = 0; totalTransactions < 100; totalTransactions++) {
    ...
    for (size_t input = std::min(InsecureRandRange(maxOutputs) + 1, uint64_t(outpoints.size())); input > 0; input--) {
      std::swap(outpoints[InsecureRandRange(outpoints.size())], outpoints.back());
    ...
    for (size_t output = InsecureRandRange(maxOutputs) + 1; output > 0; output--) {
      mtx.vout.emplace_back(10 * Amount::min_amount(), CScript() << OP_TRUE);
    }
...
TEST_CASE("MempoolRemoveTest") {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    txParent.vout[i].nValue = 33 * Amount::min_amount();
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    txChild[i].vout[0].nValue = 11 * Amount::min_amount();
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    txGrandChild[i].vout[0].nValue = 11 * Amount::min_amount();
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    testPool.addUnchecked(txGrandChild[i].GetId(), entry.FromTx(txGrandChild[i]));
  }
...
TEST_CASE("MempoolIndexingTest") {
  ...
  pool.addUnchecked(tx1.GetId(), entry.Fee(10 * Amount::min_amount()).FromTx(tx1));
  ...
  pool.addUnchecked(tx2.GetId(), entry.Fee(20 * Amount::min_amount()).FromTx(tx2));
  ...
  pool.addUnchecked(tx3.GetId(), entry.Fee(Amount::zero()).FromTx(tx3));
  ...
  pool.addUnchecked(tx4.GetId(), entry.Fee(15 * Amount::min_amount()).FromTx(tx4));
  ...
  entry.nTime = 1;
  pool.addUnchecked(tx5.GetId(), entry.Fee(10 * Amount::min_amount()).FromTx(tx5));
  ...
  pool.addUnchecked(tx8.GetId(), entry.Fee(Amount::zero()).Time(2).FromTx(tx8), setAncestors);
  ...
  pool.addUnchecked(tx9.GetId(), entry.Fee(Amount::zero()).Time(3).FromTx(tx9), setAncestors);
...
TEST_CASE("MempoolAncestorIndexingTest") {
  ...
  CheckSort<ancestor_score>(pool, sortedOrder, "MempoolAncestorIndexingTest2");
...
TEST_CASE("MempoolSizeLimitTest") {
  ...
  tx1.vout[0].nValue = 10 * COIN;
  pool.addUnchecked(tx1.GetId(), entry.Fee(10 * COIN).FromTx(tx1));
  ...
  pool.addUnchecked(tx2.GetId(), entry.Fee(5 * COIN).FromTx(tx2));
  ...
  pool.addUnchecked(tx3.GetId(), entry.Fee(20 * COIN).FromTx(tx3));
  ...
  pool.addUnchecked(tx4.GetId(), entry.Fee(7 * Amount::min_amount()).FromTx(tx4));
  pool.addUnchecked(tx5.GetId(), entry.Fee(1 * Amount::min_amount()).FromTx(tx5));
  pool.addUnchecked(tx6.GetId(), entry.Fee(1 * Amount::min_amount()).FromTx(tx6));
  pool.addUnchecked(tx7.GetId(), entry.Fee(9 * Amount::min_amount()).FromTx(tx7));
  ...
  if (!pool.exists(tx5.GetId())) pool.addUnchecked(tx5.GetId(), entry.Fee(Amount::min_amount()).FromTx(tx5));
  pool.addUnchecked(tx7.GetId(), entry.Fee(9 * Amount::min_amount()).FromTx(tx7));
  ...
  pool.addUnchecked(tx5.GetId(), entry.Fee(1 * COIN).FromTx(tx5));
  pool.addUnchecked(tx7.GetId(), entry.Fee(9 * COIN).FromTx(tx7));
  ...
  BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(), (maxFeeRateRemoved.GetFeePerK() + feeIncrement) / 2);
  // ... then feerate should drop 1/2 each halflife

  SetMockTime(42 + 2 * CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE / 2);
  BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 5 / 2).GetFeePerK(),
  ...
  BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 9 / 2).GetFeePerK(),
                    (maxFeeRateRemoved.GetFeePerK() + feeIncrement) / 8 + Amount::min_amount());
#endif
...
void CheckDisconnectPoolOrder(DisconnectedBlockTransactions &disconnectPool, std::vector<TxId> correctlyOrderedIds,
...
TEST_CASE("TestImportMempool") {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    chainedTxn[i].vout.emplace_back(10 * Amount::min_amount(), CScript() << OP_TRUE);
  ...
  vecptx disconnectedTxnsInOrder = {&chainedTxn[0], &chainedTxn[1], &chainedTxn[2]};
  vecptx disconnectedTxnsMixedOrder = {&chainedTxn[1], &chainedTxn[2], &chainedTxn[0]};
  vecptx disconnectedTxnsInvertedOrder = {&chainedTxn[2], &chainedTxn[1], &chainedTxn[0]};
  ...
  // correctlyOrderedIds.
  for (auto &disconnectedTxns : {disconnectedTxnsInOrder, disconnectedTxnsMixedOrder, disconnectedTxnsInvertedOrder}) {
    for (auto &unconfTxns : {unconfTxnsInOrder, unconfTxnsOutOfOrder}) {
      ...
      disconnectPool.importMempool(testPool);
      CheckDisconnectPoolOrder(disconnectPool, correctlyOrderedIds, disconnectedTxns.size() + unconfTxns.size());
