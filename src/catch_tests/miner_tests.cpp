...
void TestPackageSelection(Config &config, CScript scriptPubKey, std::vector<CTransactionRef> &txFirst) {
  ...
  g_mempool.addUnchecked(entry.Fee(Amount(1000)).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));

  // This tx has a medium fee: 10000 satoshis.
  tx.vin[0].prevout = COutPoint(txFirst[1]->GetId(), 0);
  tx.vout[0].nValue = Amount(5000000000LL - 10000);
  TxId mediumFeeTxId = tx.GetId();
  g_mempool.addUnchecked(entry.Fee(Amount(10000)).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));
  ...
  g_mempool.addUnchecked(entry.Fee(Amount(50000)).Time(GetTime()).SpendsCoinbase(false).FromTx(tx));
  ...
  pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey);
  ...
  lowFeeTxId = tx.GetId();
  g_mempool.addUnchecked(entry.Fee(feeToUse + Amount(2)).FromTx(tx));
  pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey);
  ...
  g_mempool.addUnchecked(entry.Fee(Amount::zero()).SpendsCoinbase(true).FromTx(tx));
  ...
  g_mempool.addUnchecked(entry.Fee(feeToUse).SpendsCoinbase(false).FromTx(tx));
  pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey);
  ...
  g_mempool.addUnchecked(entry.Fee(Amount(10000)).FromTx(tx));
  pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey);
...
void TestCoinbaseMessageEB(uint64_t eb, std::string cbmsg) {
  ...
  CScript scriptPubKey = CScript() << ParseHex("04678afdb0fe5548271967f1a67130b7105cd6a828e03909"
...
TEST_CASE("CreateNewBlock_validity") {
  TestingSetup setup;
  ...
  for (unsigned int i = 0; i < 1001; ++i) {
    ...
    // creation fails.
    g_mempool.addUnchecked(entry.Fee(LOWFEE).Time(GetTime()).SpendsCoinbase(spendsCoinbase).FromTx(tx));
  ...
  for (unsigned int i = 0; i < 1001; ++i) {
    ...
    g_mempool.addUnchecked(
                           entry.Fee(LOWFEE).Time(GetTime()).SpendsCoinbase(spendsCoinbase).SigOpsCost(80).FromTx(tx));
    tx.vin[0].prevout = COutPoint(hash, 0);
  }
  BOOST_CHECK(pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey));
  g_mempool.clear();
  ...
  hash = tx.GetId();
  g_mempool.addUnchecked(entry.Fee(HIGHFEE).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));
  tx.vin[0].prevout = COutPoint(hash, 0);
  tx.vin.resize(2);
  ...
  hash = tx.GetId();
  g_mempool.addUnchecked(entry.Fee(HIGHERFEE).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));
  BOOST_CHECK(pblocktemplate = BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey));
  g_mempool.clear();
  ...
  g_mempool.addUnchecked(entry.Fee(LOWFEE).Time(GetTime()).SpendsCoinbase(false).FromTx(tx));
#ifdef DEBUG_THIS
  BOOST_CHECK_THROW(BlockAssembler(config, g_mempool).CreateNewBlock(scriptPubKey), std::runtime_error);
#endif
  g_mempool.clear();
...

  {
    ...
    BOOST_CHECK(ContextualCheckTransactionForCurrentBlock(config, CTransaction(tx), state, flags));
  }

  // Sequence locks fail.
  BOOST_CHECK(!TestSequenceLocks(CTransaction(tx), flags));
  // Sequence locks pass on 2nd block.
  BOOST_CHECK(SequenceLocks(CTransaction(tx), flags, &prevheights, CreateBlockIndex(chainActive.Tip()->nHeight + 2)));
  ...
  BOOST_CHECK(SequenceLocks(CTransaction(tx), flags, &prevheights, CreateBlockIndex(chainActive.Tip()->nHeight + 1)));
  for (int i = 0; i < CBlockIndex::nMedianTimeSpan; i++) {
    // Undo tricked MTP.
    chainActive.Tip()->GetAncestor(chainActive.Tip()->nHeight - i)->nTime -= 512;
  }
...

  {
    ...
    BOOST_CHECK(ContextualCheckTransaction(config, CTransaction(tx), state, chainActive.Tip()->nHeight + 2,
...

  {
    ...
    BOOST_CHECK(ContextualCheckTransaction(config, CTransaction(tx), state, chainActive.Tip()->nHeight + 1,
                                           nMedianTimePast, nMedianTimePast));
  }
...
TEST_CASE("BlockAssembler_construction") {
  TestingSetup setup;
  GlobalConfig config;
  ...
  CheckBlockMaxSize(config, DEFAULT_MAX_BLOCK_SIZE - 1001, DEFAULT_MAX_BLOCK_SIZE - 1001);
  CheckBlockMaxSize(config, DEFAULT_MAX_BLOCK_SIZE - 1000, DEFAULT_MAX_BLOCK_SIZE - 1000);
  CheckBlockMaxSize(config, DEFAULT_MAX_BLOCK_SIZE - 999, DEFAULT_MAX_BLOCK_SIZE - 1000);
  CheckBlockMaxSize(config, DEFAULT_MAX_BLOCK_SIZE, DEFAULT_MAX_BLOCK_SIZE - 1000);
  ...
  // DEFAULT_MAX_GENERATED_BLOCK_SIZE
  {
    ...
    BlockAssembler ba(config, g_mempool);
    BOOST_CHECK_EQUAL(ba.GetMaxGeneratedBlockSize(), DEFAULT_MAX_GENERATED_BLOCK_SIZE - 1000);
  }
}
