...
static bool ToMemPool(const CMutableTransaction &tx) {
  ...
  return AcceptToMemoryPool(GetConfig(), g_mempool, state, MakeTransactionRef(tx), false, nullptr, true,
                            Amount::zero());
}

TEST_CASE("tx_mempool_block_doublespend") {
  ...
  CScript scriptPubKey = CScript() << ToByteVector(setup.coinbaseKey.GetPubKey()) << OP_CHECKSIG;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    uint256 hash = SignatureHash(scriptPubKey, CTransaction(spends[i]), 0, SigHashType().withForkId(),
                                 setup.coinbaseTxns[0].vout[0].nValue);
    BOOST_CHECK(setup.coinbaseKey.SignECDSA(hash, vchSig));
  ...
  block = setup.CreateAndProcessBlock(spends, scriptPubKey);
  ...
  BOOST_CHECK(ToMemPool(spends[1]));
  block = setup.CreateAndProcessBlock(oneSpend, scriptPubKey);
  BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
...
void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache,
                                    bool upgraded_nop) 
...
TEST_CASE("checkinputs_test, TestChain100Setup") {
  TestChain100Setup setup;
  ...
  // testing consensus vs non-standard rules in `checkinputs_test`.
  {
    ...
    mutableFunding_tx.vin[0].prevout = COutPoint(setup.coinbaseTxns[0].GetId(), 0);
    mutableFunding_tx.vout.resize(1);
    mutableFunding_tx.vout[0].nValue = 50 * COIN;

    CKey dummyKey;
    dummyKey.MakeNewKey();
    nulldummyPubKeyScript << OP_1 << ToByteVector(setup.coinbaseKey.GetPubKey()) << ToByteVector(dummyKey.GetPubKey())
                          << OP_2 << OP_CHECKMULTISIG;
    mutableFunding_tx.vout[0].scriptPubKey = nulldummyPubKeyScript;
    std::vector<uint8_t> nullDummyVchSig;
    uint256 nulldummySigHash = SignatureHash(p2pk_scriptPubKey, CTransaction(mutableFunding_tx), 0,
                                             SigHashType().withForkId(), setup.coinbaseTxns[0].vout[0].nValue);
    BOOST_CHECK(setup.coinbaseKey.SignECDSA(nulldummySigHash, nullDummyVchSig));
  ...
  {
    CBlock block = setup.CreateAndProcessBlock({mutableFunding_tx}, p2pk_scriptPubKey);
    BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
  ...
  {
    std::vector<uint8_t> vchSig;
    uint256 hash = SignatureHash(nulldummyPubKeyScript, CTransaction(spend_tx), 0, SigHashType().withForkId(),
                                 mutableFunding_tx.vout[0].nValue);
    setup.coinbaseKey.SignECDSA(hash, vchSig);
    vchSig.push_back(uint8_t(SIGHASH_ALL | SIGHASH_FORKID));
  ...
  // spend_tx is invalid according to NULLDUMMY
  {
    ...
    BOOST_CHECK(!CheckInputs(tx, state, pcoinsTip.get(), true, MANDATORY_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_NULLDUMMY,
    ...
    // successes.
    ValidateCheckInputsForAllFlags(tx, SCRIPT_VERIFY_NULLDUMMY, false, false);
  }
  ...
  block = setup.CreateAndProcessBlock({spend_tx}, p2pk_scriptPubKey);
  BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
  BOOST_CHECK(pcoinsTip->GetBestBlock() == block.GetHash());
  ...
  // test validity with P2SH.
  {
    ...
    ValidateCheckInputsForAllFlags(CTransaction(invalid_under_p2sh_tx), SCRIPT_VERIFY_P2SH, true, false);
  }
  ...
  // Test CHECKLOCKTIMEVERIFY
  {
    ...
    // Sign
    std::vector<uint8_t> vchSig;
    uint256 hash = SignatureHash(spend_tx.vout[1].scriptPubKey, CTransaction(invalid_with_cltv_tx), 0,
                                 SigHashType().withForkId(), spend_tx.vout[1].nValue);
    BOOST_CHECK(setup.coinbaseKey.SignECDSA(hash, vchSig));
    vchSig.push_back(uint8_t(SIGHASH_ALL | SIGHASH_FORKID));
    invalid_with_cltv_tx.vin[0].scriptSig = CScript() << vchSig << 101;

    ValidateCheckInputsForAllFlags(CTransaction(invalid_with_cltv_tx), SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true);
    ...
    BOOST_CHECK(CheckInputs(transaction, state, pcoinsTip.get(), true,
                            MANDATORY_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true, txdata,
                            nullptr));
  }
  ...
  // TEST CHECKSEQUENCEVERIFY
  {
    ...
    // Sign
    std::vector<uint8_t> vchSig;
    uint256 hash = SignatureHash(spend_tx.vout[2].scriptPubKey, CTransaction(invalid_with_csv_tx), 0,
                                 SigHashType().withForkId(), spend_tx.vout[2].nValue);
    BOOST_CHECK(setup.coinbaseKey.SignECDSA(hash, vchSig));
    vchSig.push_back(uint8_t(SIGHASH_ALL | SIGHASH_FORKID));
    invalid_with_csv_tx.vin[0].scriptSig = CScript() << vchSig << 101;

    ValidateCheckInputsForAllFlags(CTransaction(invalid_with_csv_tx), SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true);
    ...
    BOOST_CHECK(CheckInputs(transaction, state, pcoinsTip.get(), true,
                            MANDATORY_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true, txdata,
                            nullptr));
  }
...

  {
    ...
    // missing.
    BOOST_CHECK(!CheckInputs(transaction, state, pcoinsTip.get(), true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, true,
                             txdata, nullptr));
