...
TEST_CASE("SimpleRoundTripTest") {
  ...
  // Do a simple ShortTxIDs RT
  {
    ...
    BOOST_CHECK_EQUAL(pool.mapTx.find(block.vtx[2]->GetId())->GetSharedTx().use_count(), SHARED_TX_OFFSET + 1);
    ...
    CBlock block2;
    {
      ...
      BOOST_CHECK(partialBlock.FillBlock(block2, {}) == READ_STATUS_INVALID);
      partialBlock = tmp;
    }
