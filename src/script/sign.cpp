...
static bool Sign1(const CKeyID<0> &address, const BaseSignatureCreator &creator,
...
static bool Sign1(const CKeyID<1> &address, const BaseSignatureCreator &creator,
                  const CScript &scriptCode, std::vector<valtype> &ret) {
    std::vector<uint8_t> vchSig;
    if (!creator.CreateSig(vchSig, address, scriptCode)) {
        return false;
...
static bool SignN(const std::vector<valtype> &multisigdata,
    ...
    for (size_t i = 1; i < multisigdata.size() - 1 && nSigned < nRequired;
        ...
        CKeyID<0> keyID = CPubKey(pubkey).GetKeyID();
...
static bool SignStep(const BaseSignatureCreator &creator,
    ...
    switch (whichTypeRet) {
        ...
        case TX_PUBKEYHASH: {
            keyID = CKeyID<0>(uint160(vSolutions[0]));
